###########################
#
# This function is located in IKPy.src.ikpy. 
# This one is modified so that the orientation is used as well as translation.
#
###########################
...
def inverse_kinematic_optimization(chain, target_frame, starting_nodes_angles, regularization_parameter=None, max_iter=None):
    ...
    def optimize_target(x):
        ...
        diff_matrix = chain.forward_kinematics(y) - target

        diff_matrix[:3,:3] *=1          #weights on angle

        squared_distance = np.linalg.norm(diff_matrix)
    ...
    logging.info("Inverse kinematic optimisation OK, done in {} iterations".format(res.nit))
